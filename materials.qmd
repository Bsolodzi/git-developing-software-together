---
title: "Getting the most of git (and RStudio)"
editor: visual
---
...
So, we mentioned `add` and `commit`, these are git commands. If you have experience working with the command line you could use git from there, but the same commands can be executed from a GUI like [GitHub Desktop](https://desktop.github.com/) or [GitKraken](https://www.gitkraken.com/). During this workshop we'll use RStudio.
...
Before you create your first repository you need to make sure that git and RStudio are friends and that git knows you. If you follow the [pre-workshop instructions](preparation.html), RStudio, git, and GitHub are definitely talking to each other.
...
1.  Now, push the commits to the remote repository using the button with the big green arrow pointing up.
:::
...
### A new ringtone, anyone?

As a fun example will collaborate with my ringtone collection and do some pull request on the way.

::: {.alert .alert-dismissible .alert-light}
### Fork the repository

1.  Go to [github.com/paocorrales/ringtone_maker](https://github.com/paocorrales/ringtone_maker)
...
This repository is a copy of the one in the original repo. It's a very small project with a few file. `ringtone_maker.R`` includes the necessary code to create beautiful you new and very modern ringtone.

It uses the [*Rmusic*](https://github.com/keithmcnulty/Rmusic) library and some sampling to create a short song with random notes and semitones.

::: {.alert .alert-dismissible .alert-light}
### Make a change in the repo

1.  Make a copy of the `ringtone_maker.R` file and change it's name to `<your-name_ringtone.R>`

2.  Change the value of the first 3 variables in the script:

    -   `seed` there are some random things happening there.
    -   `n_notes` number of notes, it will afect the length of the song.

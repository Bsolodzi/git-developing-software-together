...
::: {.alert .alert-dismissible .alert-primary}
You can also configure git from the command line:

-   `git config --global user.name "Your Name"` to configure your name, it can be anything but remember that it's public.
-   `git config --global user.email youremail@example.com` to add your email (the same you used for the GitHub account).
:::
